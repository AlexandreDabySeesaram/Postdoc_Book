...
\subsection{Frontiere parametrisation}

If we only care about healthy and diseased zones it seems to be overkill to have several hundreds of parameters to tile the entire space. Using directly a parametrisation of the diseased boundary seems more appropriate. An example of which is illustrated in \cref{fig:space_partition} for two regions in the 1D case.

\begin{figure}
    \centering
    \includegraphics[width = 0.7\linewidth]{Figures/SpacePartition.pdf}
    \caption{Space partition}
    \label{fig:space_partition}
\end{figure}

One difficulty that may arrise from such an approach is the loss of separability in the TD for the zone parametrisation regarding space. Indeed, the space modes would then depend on those parameters. 
...
If clustering region is fine then a choice of separability for the solution must be made between
    ...
    Somehow it makes sens to first create the $\textcolor{BleuLMS!70}{\overline{\vect{u}}_i(\vect{x,\alpha})}$ rather than mix the space vriability with the parametric variability but it might not be the best solution.

    \Rq{Intuitively there is a kind of hierarchy in the parameter which I think should be reflected in the decomposition.}
