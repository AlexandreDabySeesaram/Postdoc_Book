...
\begin{algorithm}[hbtp!]
	...
	Break\textcolor{GreenLMS}{\Comment*[r]{Break if stagnation not solved by previous mode}}
}
		...
		\code{loss\_decrease} = $\frac{2\left(\text{\code{L\_old - L}}\right)}{\left|\text{\code{L\_old + L}}\right|}$ 
		...
		\If{\emph{\code{L}} $> 0$}{
			\code{usefullness} +=1 \textcolor{GreenLMS}{\Comment*[r]{ Check that loss decrease is not just a single spike}}
		...
		\code{optimiser.step()}  \textcolor{GreenLMS}{\Comment*[r]{ Update the models}}
			...
			\code{model.AddMode2Optimizer(optimizer)}
