...
\begin{algorithm}[hbtp!]
	...
	\While(\textcolor{GreenLMS}{\Comment*[r]{\hspace{-5pt}Training loop \hspace{-6pt}}}){Training}{
		 \code{\{$\sigma_i$\}} $ = f$( \code{\{$\sigma_i$\}}, \code{\{$u_i$\}})  \textcolor{GreenLMS}{\Comment*[r]{ Boundary conditions constraints}}
		 
		 $\sigma \left(x \right) = $  \code{model\_stress}$(x)$ \textcolor{GreenLMS}{\Comment*[r]{ Stress evaluation}}
		 
		 $u \left(x \right) = $  \code{model\_disp}$(x)$    \textcolor{GreenLMS}{\Comment*[r]{ Displacement evaluation}}
		 
		 $\mathcal{L} = \mathcal{L}\left(\sigma \left(x \right) , u \left(x \right)  \right)$  \textcolor{GreenLMS}{\Comment*[r]{ Loss evaluation}}
